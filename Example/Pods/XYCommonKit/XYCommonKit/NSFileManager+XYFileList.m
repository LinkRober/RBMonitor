...
{
    if ([NSString xy_isEmptyString:filePath] || [filePath length] < 2) {
        return filePath;
    }
...
+ (void)xy_copyFileNamed:(NSString *)filename intoFolder:(NSString *)dirname
{
    ...
    NSString *defaultFileFolder = [[NSBundle xy_xiaoyingBundle] resourcePath];
    
    if(!defaultFileFolder){
        defaultFileFolder = [[NSBundle mainBundle] resourcePath];
    }
...
{
    if ([NSString xy_isEmptyString:filePath]) {
        return NO;
    }
...
{
    if(![self xy_isFileExist:fromFilePath] || [NSString xy_isEmptyString:toFilePath]){
        return NO;
    }
...
+ (BOOL)xy_saveUIImage:(UIImage *)imageToSave toFilePath:(NSString *)toFilePath
{
    ...
    if([toFilePath hasSuffix:@".png"]){
        ...
        return [UIImageJPEGRepresentation(imageToSave, 0.7) writeToFile:toFilePath atomically:YES];
    }
}
...
{
    NSString *destinationTmpPath = [NSString stringWithFormat:@"%@/Temp_%lld",destinationPath,[NSDate xy_currentMilliseconds]];
