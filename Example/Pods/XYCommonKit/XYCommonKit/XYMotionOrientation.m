//
//  XYMotionOrientation.m
//
//  Created by Sangwon Park on 5/3/12.
//  Copyright (c) 2012 tastyone@gmail.com. All rights reserved.
//

#import "XYMotionOrientation.h"

#define MO_degreesToRadian(x) (M_PI * (x) / 180.0)

NSString* const XYMotionOrientationChangedNotification = @"XYMotionOrientationChangedNotification";
NSString* const XYMotionOrientationInterfaceOrientationChangedNotification = @"XYMotionOrientationInterfaceOrientationChangedNotification";

NSString* const kXYMotionOrientationKey = @"kXYMotionOrientationKey";

@interface XYMotionOrientation ()
@property (strong) CMMotionManager* motionManager;
@property (strong) NSOperationQueue* operationQueue;
@end


@implementation XYMotionOrientation
...
+ (XYMotionOrientation *)sharedInstance
{
    static XYMotionOrientation *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[XYMotionOrientation alloc] init];
    });
...
- (void)_initialize
{
    ...
    if ( ![self.motionManager isAccelerometerAvailable] ) {
        if ( self.showDebugLog ) {
            NSLog(@"XYMotionOrientation - Accelerometer is NOT available");
        }
#ifdef __i386__
        [self simulatorInit];
#endif
...
//#ifdef DEBUG
//            orientationString = @"XYMotionOrientation - FaceDown";
//#endif
//        } else {
//            newDeviceOrientation = UIDeviceOrientationFaceUp;
//#ifdef DEBUG
//            orientationString = @"XYMotionOrientation - FaceUp";
//#endif
//        }
	}
...
#ifdef DEBUG
        orientationString = @"XYMotionOrientation - Portrait";
#endif
	}
...
#ifdef DEBUG
        orientationString = @"XYMotionOrientation - Left";
#endif
	}
...
#ifdef DEBUG
        orientationString = @"XYMotionOrientation - UpsideDown";
#endif
	}
...
#ifdef DEBUG
        orientationString = @"XYMotionOrientation - Right";
#endif
	} else {
    ...
    if ( deviceOrientationChanged ) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [[NSNotificationCenter defaultCenter] postNotificationName:XYMotionOrientationChangedNotification
                                                                object:nil
                                                              userInfo:[NSDictionary dictionaryWithObjectsAndKeys:self, kXYMotionOrientationKey, nil]];
        });
    ...
    if ( interfaceOrientationChanged ) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [[NSNotificationCenter defaultCenter] postNotificationName:XYMotionOrientationInterfaceOrientationChangedNotification
                                                                object:nil
                                                              userInfo:[NSDictionary dictionaryWithObjectsAndKeys:self, kXYMotionOrientationKey, nil]];
        });
    }
...
{
    // Simulator
    NSLog(@"XYMotionOrientation - Simulator in use. Using UIDevice instead");
