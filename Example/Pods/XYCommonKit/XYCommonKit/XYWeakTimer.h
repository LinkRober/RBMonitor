...
/**
 `XYWeakTimer` behaves similar to an `NSTimer` but doesn't retain the target.
 ...
 * @param repeats if `YES`, `selector` will be invoked on `target` until the `XYWeakTimer` object is deallocated or until you call `invalidate`. If `NO`, it will only be invoked once.
...
/**
 * Creates an `XYWeakTimer` object and schedules it to start ticking inmediately.
 */
 ...
 * @note `XYWeakTimer` won't invoke the `selector` on `target` again after calling this method.
